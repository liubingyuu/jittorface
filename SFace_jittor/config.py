...
def get_config(args):
    ...
    if args.workers_id == 'cpu':
        configuration['GPU_ID'] = []
        print("check", args.workers_id)
    else:
    ...
    if args.data_mode == 'casia':
        configuration['DATA_ROOT'] = './data/faces_webface_112x112/' # the dir for training
    else:
        raise Exception(args.data_mode)

    configuration['EVAL_PATH'] = './data/faces_webface_112x112/' # the dir for validation

    assert args.net in ['IR_50', 'IR_101', 'MobileFaceNet']


    configuration['BACKBONE_NAME'] = args.net
    assert args.head in ['SFaceLoss']
